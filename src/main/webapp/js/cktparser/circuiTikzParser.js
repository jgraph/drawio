    ...
    circuitMap.forEach((value, key, map) => {
        if (value instanceof CktLine) { // Check first, b/c also returns true for 'value instanceof CktPath'
            for (let i = 0; i < value.vertices.length; i++) {
                if (parseFloat(value.vertices[i].x) < minX) {
                    minX = parseFloat(value.vertices[i].x);
                }
                if (parseFloat(value.vertices[i].y) > maxY) {
                    maxY = parseFloat(value.vertices[i].y);
                }
            }
            ...
            for (let i = 0; i < rotatedVertices.length; i++) {
                if (parseFloat(rotatedVertices[i].x) < minX) {
                    minX = parseFloat(rotatedVertices[i].x);
                }
                if (parseFloat(rotatedVertices[i].y) > maxY) {
                    maxY = parseFloat(rotatedVertices[i].y);
                }
            }
        }
    });
