 ...
 * Gitlab Client ID
 */
GitLabClient.prototype.clientId = '66a735ad3dd00fe91096942703ca3851a2cf61e91859efba1039bf4319f1cee2';
...
GitLabClient.prototype.authenticate = function(success, error)
{
	if (window.onGitLabCallback == null)
	{
		var auth = mxUtils.bind(this, function()
		{
			...
			{
				var state = '123';
				var redirectUri = encodeURIComponent('http://localhost:8080/gitlab.html');
...
GitLabClient.prototype.getFile = function(path, success, error, asLibrary, checkExists)
{
	...
	var tokens = path.replace(/^ITLAB/ig, '').split('/');
	var org = tokens[0];
	var repo = tokens[1];
	var ref = tokens[2];
...
GitLabClient.prototype.createGitLabFile = function(org, repo, ref, data, asLibrary)
{
	...
	var meta = {'org': org, 'repo': repo, 'ref': ref, 'name': data.file_name,
...
GitLabClient.prototype.pickFile = function(fn)
{
	...
	{
		this.ui.loadFile('GITLAB' + encodeURIComponent(path));
	});
