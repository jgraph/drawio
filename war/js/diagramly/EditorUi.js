	...
	Graph.prototype.compress = function(data)
	{
		...
		else
		{
			...
			try
	    	{
	    		...
	    		result = (window.btoa) ? btoa(tmp) : Base64.encode(tmp, true);
	    	}
			catch (e)
			{
				// TODO: Remove for Atlas
		    	if (!mxClient.IS_CHROMEAPP)
	    		{
	    			mxUtils.post('/email', 'email=drawio-app' +
						'&version=' + encodeURIComponent(EditorUi.VERSION) +
						'&url=' + encodeURIComponent(window.location.href) +
						'&data=' + encodeURIComponent('COMPRESS ERROR\n' +
						'Error: ' + e + '\nData: ' + data));
	    		}
	...
	Graph.prototype.decompress = function(data)
	{
		...
		else
		{
			...
			try
	    	{
				...
				result = this.zapGremlins(decodeURIComponent(
			...
			catch (e)
			{
		    	...
		    	if (!mxClient.IS_CHROMEAPP)
	    		{
					mxUtils.post('/email', 'email=drawio-app' +
						'&version=' + encodeURIComponent(EditorUi.VERSION) +
						'&url=' + encodeURIComponent(window.location.href) +
						'&data=' + encodeURIComponent('DECOMPRESS ERROR\n' +
						'Error: ' + e + '\nData: ' + data));
	    		}
